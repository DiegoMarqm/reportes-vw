...
const form = useForm({
    numFolio: props.reporte.numFolio || '',              //Campo Numero de Folio
    departamento: props.reporte.departamento || '',      // Campo Departamento
    calificacion: props.reporte.calificacion || '',      // Campo Calificación
    fechaQueja: props.reporte.fechaQueja || '',          // Campo Fecha de la queja
    fechaEntrega: props.reporte.fechaEntrega || '',      // Campo Fecha de entrega de la unidad puede estar vacio
    fechaIngreso: props.reporte.fechaIngreso || '',      // Campo Fecha de ingreso puede estar vacio
    formaDeteccion: props.reporte.formaDeteccion || '',  // Campo Forma que se detectó la queja
    redSocial: props.reporte.redSocial || '',            // Campo Red social puede ir vacio
    nomCliente: props.reporte.nomCliente || '',          // Campo Nombre del Cliente
    celularCliente: props.reporte.celularCliente || '',  // Campo Celular
    emailCliente: props.reporte.emailCliente || '',      // Campo E-mail
    asesor: props.reporte.asesor || '',                  // Campo Asesor/Ejecutivo
    tipoModelo: props.reporte.tipoModelo || '',          // Campo Tipo/Modelo
    placas: props.reporte.placas || '',                  // Campo Placas puede ir vacio
    color: props.reporte.color || '',                    // Campo Color
    noOrden: props.reporte.noOrden || '',                // Campo No. Orden
    vin: props.reporte.vin || '',                        // Campo No. de VIN
    tecnico: props.reporte.tecnico || '',                // Campo Técnico
    reclamacion: props.reporte.reclamacion || '',        // Campo Reclamación
    tipoReclamacion: props.reporte.tipoReclamacion || '',// Campo Tipo de reclamación
    otroTipoReclamacion: props.reporte.otroTipoReclamacion || '',  //Campo de Otro tipo de reclamacion puede ir vacio

    // Causa Raiz va a ser un Array en el modelo y en la migracion esta indicado como tipo json y puede ir nulo o no
    causaRaiz: {
        personas: props.reporte.causaRaiz?.personas || '',
        proceso: props.reporte.causaRaiz?.proceso || '',
        equipo: props.reporte.causaRaiz?.equipo || '',
        material: props.reporte.causaRaiz?.material || '',
        entorno: props.reporte.causaRaiz?.entorno || '',
        administracion: props.reporte.causaRaiz?.administracion || '',
    },

    medidas: props.reporte.medidas || [],

    procedeQueja: props.reporte.procedeQueja || false,   // Campo Procede la queja boleano
    solucion: props.reporte.solucion || '',              // Campo Solución es un text
    nombreCierre: props.reporte.nombreCierre || '',      // Campo Nombre del personal de cierre es un select
    fechaCierre: props.reporte.fechaCierre || '',        // Campo Fecha de cierre es un date
    nombreSeguimiento: props.reporte.nombreSeguimiento || '', //Campo Nombre del personal que hace el seguimiento
    fechaSeguimiento: props.reporte.fechaSeguimiento || '',   //Campo fecha del seguimiento
    comentariosCliente: props.reporte.comentariosCliente || '', //Campo de comentarios del cliente
});
        ...
        <div class="p-4 space-y-8 font-vwtext">
            <form @submit.prevent="add" class="bg-white rounded-lg shadow p-6 space-y-6">
                ...
                <div class="flex justify-end">
                    <Link :href="route('reporte.show', props.reporte.id)"
                        class="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-700 mr-2">Cancelar
                    </Link>

                    <button type="submit" @click="editReporte"
                        class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-700">Enviar
                    </button>
